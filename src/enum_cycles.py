...
def load_graph(input_file: str|None, complete: int|None, multicycle: tuple|None) -> DiGraph:
...
def enumerate_cycles(graph: DiGraph, nx: bool, hj: bool, wein: bool) -> list:
    if nx:
...
def main():
    ...
    graph = load_graph(args.input, args.complete, args.multicycle)
    # Get cycles with an internal representation of vertices
    cycles_inter_repr = enumerate_cycles(graph, args.nx, args.hj, args.wein)
