...
\projectinfo{
  ...
  title={Repetitive Substructures for \\Efficient Representation of Automata}, % Název prezentace / Presentation title (The whole title of the presented work suitably divided into lines that are optically balanced)
  title.footer={Repetitive Substructures for Efficient Representation of Automata}, % Název prezentace - text zobrazovaný vedle čísla slajdu / Presentation title - displayed next to the slide number (The full title of the presented work, it may be suitably abbreviated to fit the footer)
  author.name={Michal},  % Jméno autora / Author name
  author.surname={Šedý}, % Příjmení autora / Author surname
...
\usetikzlibrary{calc}
\tikzset{rectangular state/.style={draw, rounded rectangle, inner sep=2mm, minimum height=0.7cm}}
...
\newcommand{\circledtext}[2][red]{%
    \tikz[baseline=(char.base)]{
        \node[shape=circle,draw,fill=#1,inner sep=2pt] (char) {\hspace*{0.15mm}\textbf{#2}};}\hspace*{-2mm}
}
