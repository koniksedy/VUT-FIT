...
\begin{columns}
    \column{0.4}
    \block{Motivation} {
        In many automata, especially in those representing regular expressions, there exist repetitive substructures, that can no be eliminated by state of the art reduction tool RABIT/Reduce tool \cite{RABIT} as shown in Figure \ref{fig:http}.
    ...
    \note[
        ...
        {\small\textit{Usage of a pushdown automaton and procedures is an analogy to call stack and functions from programming languages. }}
...
\begin{columns}
    ...
    \block{Parametric Regular Expressions}
    {
        ...
        \begin{tikzfigure}[\textsc{Reduction ratios achived after utilizing procedures in results of the RABIT/Reduce tool. On average, the procedures improved reduction by 50.3\% in states and by 47.9\% in transitions.}\par]
            \centering
    ...
    % \calloutblock[20]{20}{coordinate}{Block Width}{Block Content}
    ...
    \block[
        ...
        \begin{tikztable}[\textsc{Reduction results of RABIT/Reduce (RAB) and procedures (Proc) on seven sets of Snort rules. $Q$ denotes the number of states $\delta$ the number of transitions, and $\Gamma$ the number of stack symbols. The percentages refer to RABIT/Reduce results.}\par]
            \centering
            \small
        ...
        The two most significant reductions are highlighted in Table~\ref{table:snort}. The best reduction was attained on the automaton created from the \texttt{specific-threats} rule set. The RABIT/Reduce reduced the automaton by 48.6\% of states and by 44.3\% of transitions. By further application of procedures, another 43.5\% reduction in states and 60.3\% reduction in transitions was achieved. This experiment shows, that the procedures performs significant reduction even on real-world examples.
    }
    \block{References} {
        \vspace*{-2em}
        \small
