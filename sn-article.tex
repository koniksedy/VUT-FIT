...
\section{Preliminaries}
    ...
    \subsection{Binary Decision Diagram}
        ...
        \begin{figure}[h]
            ...
            \begin{subfigure}{0.25\textwidth}
                ...
                \begin{tikzpicture}[node distance=0.4cm and 0.3cm]\footnotesize
                 ...
                 \end{tikzpicture}
            \end{subfigure}
            \caption{From left to right, an OBDD satisfying the first, second, and third conditions as specified by the definition of ROBDD.}
        \end{figure}
        ...
        \begin{theorem}
            \label{theorem:ROBDD}
            For every boolean function $\phi$ over some set of variables $V\!ar$ and every variable ordering $\prec$ on $V\!ar$, there is a unique (up to isomorphism) reduced OBDD (with respect to $\prec$) $G_\phi$ which represents $\phi$. \cite{BDD}
        \end{theorem}
...
\section{Automata representation}
    ...
    \subsection{Automaton with Skip Edges}
        ...
        \begin{figure}[h]
            ...
            \end{tikzpicture}
            \caption{An automaton with skip edges representing the language from Figure \ref{fig:mona}. A skip edge labeled with $a/n$ can be interpreted as a transition of length $n$ over a symbol $a$.}
        \end{figure}
...
\section{Automata operations}
    ...
    \subsection{Intersection}
        ...
        \begin{algorithm}
            ...
            \KwData{Two DFA $M = (Q_M, \Sigma, \delta_M, i_M, F_M)$ and $N = (Q_N, \Sigma, \delta_N, i_N, F_N)$ and tow index functions $\iota_M : Q_M \rightarrow \mathbb{N}_0$ and $\iota_N : Q_N \rightarrow \mathbb{N}_0$}
            \KwOut{Resulting DFA $A = (Q \subseteq Q_M \times Q_N, \Sigma, \delta, i, F) = M \cap N$ and index function $\iota : Q \rightarrow \mathbb{N}_0$}
            ...
            \SetKwProg{procB}{Procedure}{}{}
                \procB{\textnormal{CreateState($x$, $y$)}}{
            ...
            \While{$worklist \neq \emptyset$}{
                ...
                \uIf{$\iota_M(s_M) = \iota_N(s_N)$}{
                    \ForAll{$a \in \Sigma \cap \Sigma : \delta_M(s_M, a) \in Q_M \land \delta_N(s_N, a) \in Q_N$}{
                        $\delta(s, a) \leftarrow (\delta_M(s_M, a), \delta_N(s_N, a))$\;
                        $CreateState(\delta_M(s_M, a), \delta_N(s_N, a))$\;
                    }
                }\uElseIf{$(\iota_M(s_M) < \iota_N(s_N) \land \iota_M(s_M) \neq 0) \lor \iota_N(s_N) = 0$}{
                    \ForAll{$a \in \Sigma : \delta_M(s_M, a) \in Q_M$}{
                        $\delta(s, a) \leftarrow (\delta_M(s_M, a), s_N)$\;
                        $CreateState(\delta_M(s_M, a), s_N)$\;
                    }
                }\Else{
                    \ForAll{$a \in \Sigma : \delta_N(s_N, a) \in Q_N$}{
                        $\delta(s, a) \leftarrow (s_M, \delta_N(s_N, a))$\;
                        $CreateState(s_M, \delta_N(s_N, a))$\;
                    }
                }
            }
            \Return{$A, \iota$}\;
        ...
        \begin{algorithm}
            ...
            \If{$id = 0$}{
                ...
                \ForAll{$q \in Q_M$}{
                    ...
                    $succ \leftarrow \{r \in Q_M\,|\,\exists a \in \Sigma :
 r \in \delta_M(q, a)\}$\;
                    \ForAll{$a \in \Sigma$}{
                        $\delta(q, a) \leftarrow succ$\;
                    }
                }
                \Return{$A, \iota$};
            }
            ...
            \ForAll{$q \in Q_M$}{
                ...
                $succ \leftarrow \{r \in Q_M\,|\,\exists a \in \Sigma : r \in \delta_M(q, a) \land \iota_M(r) \neq id\}$\;
