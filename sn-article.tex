...
\section{Automata operations}
    This section presents operations on automata with indexed states used in the reimplementation of Mona using Mata library. The main concept of operations is taken from Mata with a minification fot the states indexing.

    \subsection{Complement}
        The complementation is the most straightforward automaton method. It takes complete deterministic automaton and swaps its accepting and nonacceptance states. The difference from classical complementation is that only the accepting property of states with index $0$ is changed.
    ...
    The standard intersection algorithm generates the resulting automaton using product construction. It starts with the pair of initial states (one from each automaton) and moves synchronously to the pair of successors based on the transition symbol. The key idea is inspired by the \textit{Apply} method\cite{APPLY}, commonly employed in Binary Decision Diagram (BDD) operations. In this adaptation of the standard intersection algorithm, the successor pair for the analyzed product pair is generated based on the state (or states in the case of identical indices) with the lower index. Meanwhile, the state with the higher index is duplicated.

    \begin{algorithm}
        \caption{An Intersection Algorithm}
        \small
        \DontPrintSemicolon
        \SetKwFunction{proc}{get_new_index}
        ...
        \KwOut{Resulting DFA $A = (Q \subseteq Q_M \times Q_N, \Sigma \subseteq \Sigma_M \cup \Sigma_N, \delta, i, F) = M \cap N$}

        \vspace*{1em}

        \SetKwProg{proc}{Procedure}{}{}
            \proc{\textnormal{Index($s_M$, $s_N$)}}{
                \If{$\iota_N(s_N) = 0$}{
                    \Return{$\iota_M(s_M)$}\;
                }\If{$\iota_M(s_M) = 0$}{
                    \Return{$\iota_N(s_N)$}\;
        ...
        \While{$worklist \neq \emptyset$}{
            ...
            \uIf{$\iota_M(s_M) = \iota_N(s_N)$}{
                \ForAll{$a \in \Sigma_M \cap \Sigma_N : \delta_M(s_M, a) \in Q_M \land \delta_N(s_N, a) \in Q_N$}{
                    ...
                    $worklist \leftarrow worklist \cup \{(\delta_M(s_M, a), \delta_N(s_N, a))\}$\;
                    $\iota((\delta_M(s_M, a), \delta_N(s_N, a))) \leftarrow Index(\delta_M(s_M, a), \delta_N(s_N, a))$\;
                }
                ...
                \ForAll{$a \in \Sigma_M : \delta_M(s_M, a) \in Q_M$}{
                    ...
                    $worklist \leftarrow worklist \cup \{(\delta_M(s_M, a), s_N)\}$\;
                    $\iota((\delta_M(s_M, a), s_N)) \leftarrow Index(\delta_M(s_M, a), s_N)$\;
                }
            }\Else{
                \ForAll{$a \in \Sigma_M : \delta_M(s_M, a) \in Q_M$}{
                    $\delta(s, a) \leftarrow (s_M, \delta_N(s_N, a))$\;
                    $worklist \leftarrow worklist \cup \{(s_M, \delta_N(s_N, a))\}$\;
                    $\iota((s_M, \delta_N(s_N, a))) \leftarrow Index(s_M, \delta_N(s_N, a))$\;
                }
            }
        }
        \Return{$A$}\;
    ...
    \subsection{Determinization}
        Standard determinization algorithm generates resulting automaton using subset construction. Starting with the set of initial states and creating transition using one alphabet symbol to the set of successors. This method has to be modified for use with index function similarly as the intersection. The successor of the resulting state (representing the set of states) is a set created by the states from current set with the lowest index. Meanwhile, the other states are duplicated.
    ...
    \subsection{Union}
        The only difference between the union of two NFAs and the union of two NFAs with index functions is that we have to unite the functions as well.
    ...
    \subsection{Projection}
        The basic idea of a projection is to remove all transitions going from the states with the index of variable being removed and redirect all incoming edges into its successors. Only exception is projection of the variable with the index 0. In that case the redirection can not be done due to the restriction that states with the index 0 cannot be jumped over. The solution to this problem is to create edge for every letter of the alphabet and avery successor of the examined state.
    ...
    \subsection{Minimization}
        The fast minimization for incomplete deterministic automata \cite{Minimization} performing in $\mathcal{O}(m\, \text{log}\, n)$ time has been implemented. Unfortunately the benchmarks have shown that the minimization have to be done not only according the forward language equivalence, but also by the backward language equivalence. This algorithm can not be used for this task, because even it is DFA it still can have nondeterministic transitions in the backward direction.

        Since this algorithm can not be chosen for the automata minimization and the minimization algorithm based on the simulation provided by the Mata library has been too slow, the naive Brzozowski \cite{Brzozowski} algorithm has been chosen. It minimized DFA by reverting and determinizing the input automaton and then reverting and determinizing it again.
