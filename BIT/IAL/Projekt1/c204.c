...
/**
 * Projekt vypracoval: Michal Šedý (xsedym02)
 */
...
void untilLeftPar ( tStack* s, char* postExpr, unsigned* postLen ) {
	...
	while (!stackEmpty(s) && postExpr[(*postLen)-1] != '(')
	{
		// Prevod znaku z vrcholu zasobniku do postExpr na pozici postLen
		stackTop(s, &postExpr[(*postLen)++]);		
		stackPop(s);

	}

	// Nakonec odstracním i levou zavorku
	if (postExpr[(*postLen)-1] == '(')
	{
...
void doOperation ( tStack* s, char c, char* postExpr, unsigned* postLen ) {
	...
	// Je li na vstupu operand, vypiš jej na vystup
	...
	/* Operator vloz na zasobnik v pripadě:
					a) zasobnik je prazdny
					b) na vrcholu  je (
					c) ne vrcholu je operator s nizsi prioritou
			Je-li na vrcholu s vyssi nebo shodnou prioritou, odstran ho a vloz na vystup.
			Postup opakuj, dokud se ti nepodari vlozit operator.
	*/
	else if (c == '+' || c == '-' || c == '*' || c == '/')
	{
		while (1)
		{
			if(stackEmpty(s) || top == '(')	// Tento způsob jde jen diky zkratovitemu vyhodnocovani
			{
				stackPush(s, c);
				break;
			}
	...
	// Po nalezeni pravé zavorky vyprazdnime zasobnik až po levou zavorku
	else if (c == ')')
	{
		untilLeftPar(s, postExpr, postLen);
	}
	// Pokud nalezname = vyprazdnime cely zasobnik
	else if (c == '=')
	{
		while(!stackEmpty(s))	// Pro jistotu
		{
			untilLeftPar(s, postExpr, postLen);
		}
	}
...
char* infix2postfix (const char* infExpr) {
	...
	// Alokace výstupního pole, jeho nulování a kontrola
	char *output = (char*)calloc(MAX_LEN, 1);
	if (output == NULL)
	{
		return NULL;
	}
	...
	// Zpracováváni jednotlivých znaků z leva do prava.
	for (int i = 0; i < MAX_LEN && infExpr[i] != '\0'; i++)
	{
		
		doOperation(&stack, infExpr[i], output, &output_len);

		// Nakonec potřeba přidat =		
		if (infExpr[i] == '=')
		{
			output[output_len] = '=';
			break;
