...
**/
	...
	tBTNodePtr tmp_ptr = *RootPtr;

	while(tmp_ptr != NULL)
	{
		// Prohledáváme
		where = tmp_ptr;	// Potenciální uzel
	...
	if(where == NULL)
	{
		// Vkládání do prázdného BT
		*RootPtr = new;
	}
	else
	{
		// Rozhodneme o vložení vlevo, nebo vpravo
...
**/
	...
	while(!SEmptyP(&stack))
	{
		tmp_ptr = STopPopP(&stack);
		// Najít nejlevéjší pravého
		Leftmost_Preorder(tmp_ptr->RPtr, &stack);
	}
}
...
**/
	while(ptr != NULL)
	{
		// Procházíme BT po levé větvi a dáme na zásobník
		SPushP(Stack, ptr);
		ptr = ptr->LPtr;
	}
}
...
**/
	...
	while(!SEmptyP(&stack))
	{
		tmp_ptr = STopPopP(&stack);
		BTWorkOut(tmp_ptr);
		// Najít nejlevější pravého
		Leftmost_Inorder(tmp_ptr->RPtr, &stack);
	}
}
...
**/
	...
	while(ptr != NULL)
	{
		// Procházíme po levé větvi a plníme zasobníky
		SPushP(StackP, ptr);
		SPushB(StackB, true);
		ptr = ptr->LPtr;
	}
...
**/
	...
	{
		// Vytáhneme data ze zásobníků
		tmp_ptr = STopPopP(&StackP);
		SPushP(&StackP, tmp_ptr);		
		...
		{		
			// Přišli jsme zleva	
			SPushB(&StackB, false);
			Leftmost_Postorder(tmp_ptr->RPtr, &StackP, &StackB);
		}
		else
		{
			// Jdeme zpět zprava
...
**/
	...
	Leftmost_Inorder(*RootPtr, &stack);
	// Podobně jako inorder, teď ale nevypisujeme. Mažeme.
	while(!SEmptyP(&stack))
	{
		tmp_ptr = STopPopP(&stack);
