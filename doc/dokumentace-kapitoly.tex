...
\chapter{Brute-force}
    ...
    V algoritmu se používá indexace pomocí záporných čísel. Tato indexace má následující význam: index -1 značí poslední prvek listu, index -2 předposlední a tak dále. Pro sjednocení dvou cest $P_1 = \langle v_1v_2 \rangle$ a $P_2 = \langle v_3v_4 \rangle$ bude využíván operátor $+$, tedy $P_1 + P_2 = \langle v_1v_2v_3v_4 \rangle$.
        ...
        \begin{algorithm}[!h]
            ...
            $H[0 \dots n-1][0 \dots n-1] \leftarrow 0$ \tcp*{nulová matice $n \times n$}
            \For{$p_s \in V$}
            {
                $P \leftarrow  \mathit{EmptyList}$\;
                $P.append(p_s)$\;
                $p_{curr} \leftarrow p_s$\;
                % $isDone \leftarrow$ \textbf{False}\;
                \While{\textnormal{\textbf{True}}}
                {
                    $foundSuccessor \leftarrow$ \textbf{False}\;
                    \For{$(p_{curr},p_{next}) \in E$}
                    {
                        \If{$H[p_{curr}][p_{next}]=0 \land p_{next} \notin P \land p_{next} > p_s$}
                        {
                        $P.append(p_{next}$)\;
...
\chapter{Hongbo Liu a Jiaxin Wang}
    ...
    \section{Časová složitost}
        ...
        \begin{proof}
            Časová složitost zanedbání uzlů je $\mathcal{O}(|V| + |E|)$. Inicializace a naplnění fronty (1--4) $Q$ má časovou složitost $\mathcal{O}(|V|)$. Halvní cyklus (5--18) bude proveden tolikrát, kolik existuje cest $P$ odpovídajících podmínce  $\forall v \in P: ord(v) \geq ord(HEAD(P))$. Takových cest může být až $X$, kde
    ...
    \section{Prostorová složitost}
        ...
        \begin{proof}
            Do fronty $Q$ se postupně ukládají všechny cesty v grafu. Do fronty může být celkem uloženo až $X$ cest, kde
...
\chapter{Návrh programu}
    ...
    \section{Použité knihovny}
        Program využívá knihovnu Networkx (verze 2.8.7) pro porovnávání efektivity implementovaných algoritmů, knihovnu Bidict (verze 0.22.0) implementující obousměrný slovník využívaný pro obousměrný překlad jmen uzlů na jejích pořadová čísla a knihovnu Numpy (verze 1.23.3) k~reprezentaci matic. Tyto závislosti lza nainstalovat pomocí nástroje pip3, nebo do virtuálního prostředí \texttt{gal-env} pomocí skriptu \texttt{build.sh} umístěného v kořenové složce projektu.

        Dále je využita knihovna Argparse, pro zpracování argumentů příkazové řádky, která je ale součásti standardních knihoven python již od verze 3.2.
...
\chapter{Použití programu}
    ...
    \section{Instalace závislostí}
        Potřebné knihovny využívané programem lze instalovat pomocí vástroje pip3 do virtuálního prostředí \texttt{gal-env} spuštěním skriptu \texttt{build.sh}. Spušťení virtuálního prostředí lze provést příkazem \texttt{source ./gal-env/bin/activate} v kořenové úrovni projektu.
    ...
    \section{Spuštění programu}
        ...
        Dále lze zpracovávat graf zadaný vstupním souborem, nebo vygenerovaný (úplný, multi-cyklický, nebo nested). Právě jeden vstupní graf musí být zadán.

        Program \texttt{enum\_cycles.py} umístěný ve složce \texttt{/src} lze po úspěšné instalaci závislostí spustit ve virtuálním prostředí \texttt{gal-env} s využitím interpretu python3 (verze 3.8+):
    ...
    \section{Automatické testy}
        ...
        Za účelem vyšší přehlednosti je během testování tištěna informace o počtu provedených testů pomocí knihovny \texttt{tqdm}\footnote{Tqdm dostupné z: \url{https://tqdm.github.io}} (verze 4.64.1). Tato knihovna je zahrnuta v souboru závislostí \texttt{requirements.txt}.
...
\chapter{Experimenty}
    ...
    \section{Multi-cyklické grafy}
        ...
        Poměr spotřebovaného času algoritmem BF k ostatním algoritmům byl pro multi-cyklické grafy podobný jako pro úplné grafy. Z tabulky lze vypozorovat, že paměťová obtížnost algoritmu BF rostla pomaleji než u algoritmů WEIN a HJ, ale rychleji než pro algoritmus NX.
