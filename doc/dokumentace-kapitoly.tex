...
\chapter{Použití programu}
    \label{chapter:Help}
    V této kapitole je popsána struktura složky projektu. Dále je uveden postup pro instalaci knihoven (Bidict\footnote{Bidict dostupné z: \url{https://pypi.org/project/bidict}} a Networkx\footnote{Networkx dostupné z: \url{https://networkx.org}}) používaných programem. Kapitola také uvádí nápovědu pro spuštění programu pomocí interpretu Python 3 včetně vzorového formátu vstup a výstupu. Na konci kapitoly jsou uvedeny vybrané příklady jednotlivých možností spuštění.
